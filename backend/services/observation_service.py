...
SUB_MAX_CHARS  = 12   # subtitle max length
...
class ObservationService:
    ...
    async def _rewrite_vo_if_needed(
        ...
        """
        若 voice_over 超過 max_chars，要求模型重寫縮短（禁止 substring 截斷）。
        最多重試 MAX_VO_REWRITES 次；若仍超長，保留最短版本並記錄 CRITICAL。
        """
        ...
        for attempt in range(1, MAX_VO_REWRITES + 1):
            ...
            try:
                ...
                if len(rewritten) <= max_chars:
                    ...
                    return rewritten
                logger.warning(f"🔄 第 {attempt} 次重寫仍超長（{len(rewritten)}字），繼續...")
                current = rewritten
        ...
        logger.critical(
            f"❌ VO 重寫 {MAX_VO_REWRITES} 次後仍超長，"
            f"保留最短版本（{len(current)}字）: {current!r}"
        )
        return current
...
## 字幕（subtitle_zh）規則 — 旁白濃縮版
- 字數嚴格 ≤ {SUB_MAX_CHARS} 字（由 voice_over_zh 濃縮而來，不得超字）
- 保留旁白的核心意義，更精煉有力，可直接疊於畫面
- ✅ "你不懂它"、"等等——什麼？"、"原來是這個"、"這才是真相"
- ❌ 超過 {SUB_MAX_CHARS} 字、與旁白完全重複、與主題割裂
...
"""
  ...
  "units": [
    {{
      ...
      "voice_over_zh": "針對「{topic}」定位幕創作的旁白金句（≤{vo_max}字，可直接朗讀）",
      "subtitle_zh": "旁白濃縮版（≤{SUB_MAX_CHARS}字）",
    ...
    {{
      ...
      "voice_over_zh": "針對「{topic}」解構幕創作的旁白金句（≤{vo_max}字，揭露但保留答案）",
      "subtitle_zh": "旁白濃縮版（≤{SUB_MAX_CHARS}字）",
    ...
    {{
      ...
      "voice_over_zh": "針對「{topic}」影響幕創作的旁白金句（≤{vo_max}字，情感落點，非CTA）",
      "subtitle_zh": "旁白濃縮版（≤{SUB_MAX_CHARS}字）",
...
7. subtitle_zh：≤{SUB_MAX_CHARS}字，由旁白濃縮而來，保留核心意義，可直接疊於畫面
    ...
    async def generate_units(
        ...
        try:
            ...
            for idx, unit_data in enumerate(units_data):
                ...
                # VO：禁止 substring 截斷，超長時要求模型重寫
                vo_max = self._vo_max_chars(unit_duration)
                vo = unit_data.get("voice_over_zh", "")
                if len(vo) > vo_max:
                    unit_data["voice_over_zh"] = await self._rewrite_vo_if_needed(
                        vo, vo_max, notes, unit_data.get("unit_role", "解構")
                    )

                # SUB：由旁白濃縮，≤ SUB_MAX_CHARS 字
                if len(unit_data.get("subtitle_zh", "")) > SUB_MAX_CHARS:
                    unit_data["subtitle_zh"] = unit_data["subtitle_zh"][:SUB_MAX_CHARS]
